...
export const Route = createFileRoute("/auth")({
  head: () => ({ meta: [{ title: "Вход — UKA Messenger" }, { name: "description", content: "Войдите в UKA Messenger." }] }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  async function verifyCode(e: React.FormEvent) {
    ...
    setLoading(false);
    if (error) { toast.error("Неверный код"); return; }
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center px-4 py-12">
      <Link to="/" className="absolute top-6 left-6 text-sm text-muted-foreground hover:text-foreground">← Назад</Link>

      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-block float-slow"><Logo size={72} /></div>
        ...
        <div className="glass-strong rounded-2xl p-8 space-y-5">
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <div className="h-px flex-1 bg-border" /> или email <div className="h-px flex-1 bg-border" />
          </div>
          ...
          {step === "email" ? (
            <form onSubmit={sendCode} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="you@example.com" className="mt-1.5 h-12"
                />
              </div>
              <Button type="submit" disabled={loading} className="w-full h-12 bg-gradient-to-r from-[var(--neon-violet)] to-[var(--neon-cyan)] text-white font-semibold">
            ...
            <form onSubmit={verifyCode} className="space-y-4">
              <div>
                ...
                <Input
                  inputMode="numeric" required value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))}
                  placeholder="6-значный код" className="mt-1.5 h-12 text-center text-xl tracking-[0.5em] font-mono"
                  maxLength={6}
                />
              </div>
              <Button type="submit" disabled={loading || code.length < 6} className="w-full h-12 bg-gradient-to-r from-[var(--neon-violet)] to-[var(--neon-cyan)] text-white font-semibold">
