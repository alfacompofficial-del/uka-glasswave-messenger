...
function SettingsPage() {
  ...
  async function save() {
    if (!user) return;
    const e1 = validateName(firstName);
    if (e1) return toast.error("Имя: " + e1);
    const e2 = validateName(lastName);
    if (e2) return toast.error("Фамилия: " + e2);
    if (username) {
      const e3 = validateUsername(username);
      if (e3) return toast.error("Юзернейм: " + e3);
    }
    setSaving(true);
          ...
          <div className="glass-strong rounded-2xl p-6 flex items-center gap-5">
            <div className="relative">
              <Avatar className="h-24 w-24 ring-4 ring-[var(--neon-violet)]/60">
                ...
                <AvatarFallback className="text-2xl bg-gradient-to-br from-[var(--neon-violet)] to-[var(--neon-cyan)] text-white">
                  {initials}
                </AvatarFallback>
              </Avatar>
          ...
          <div className="glass-strong rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                5–32 символа: A–Z, a–z, 0–9, _. Первый — буква, без __ и без _ по краям.
              </p>
            </div>
            ...
            <Button
              onClick={save}
              disabled={saving}
              className="bg-gradient-to-r from-[var(--neon-violet)] to-[var(--neon-cyan)] text-white"
            >
