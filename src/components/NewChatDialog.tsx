      ...
      <DialogContent className="glass-strong border-border/40 max-w-md">
        ...
        <form onSubmit={(e) => { e.preventDefault(); search(); }} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="@username или телефон" className="pl-9" />
          </div>
          <Button type="submit" disabled={loading}>{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Найти"}</Button>
        </form>
        <div className="max-h-80 overflow-y-auto space-y-1 mt-2">
          {results.map((u) => {
            const initials = `${u.first_name?.[0] ?? ""}${u.last_name?.[0] ?? ""}`.toUpperCase() || "?";
            return (
              <button key={u.id} onClick={() => startChat(u.id)} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-white/5 transition text-left">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground truncate">@{u.username ?? "—"} · {u.phone ?? ""}</div>
                </div>
              </button>
            );
          })}
          {!loading && results.length === 0 && q && <div className="text-center text-sm text-muted-foreground py-6">Ничего не найдено</div>}
        </div>
      </DialogContent>
    </Dialog>
  );
